...
export default class SendApi {
    ...
    */
     ...
     * @param {String} opts.idempotencyKey Optional client-generated key that makes the submission idempotent. Retrying the same request with the same key within 24 hours returns the original response instead of creating a duplicate invoice.
     ...
     * @param {module:model/String} [latestState] Filter by the most recent SDI state for the invoice. Matches the `latest_state` field exposed inline on each Send.
     ...
     * @param {module:model/String} opts.latestState Filter by the most recent SDI state for the invoice. Matches the `latest_state` field exposed inline on each Send.
     * @param {Boolean} opts.includePayload Include payload in the response. Defaults to false.
     * @param {String} opts.ids Comma-separated list of Send ids (max 100). Filters the collection to the matching rows; unknown or unauthorized ids are silently skipped.
